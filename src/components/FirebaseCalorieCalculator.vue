<template>
  <div>
    ...
    <table class="w-100 mw5 center collapse">
      ...
      <tbody>
        <tr class="stripe-dark" v-for="meal in meals" :key="meal['.key']">
          <td class="pa2">{{meal.description}}</td>
          <td class="pa2 tr">{{meal.calories}}</td>
          <td class="pa2 tr">
            <span @click="deleteMeal(meal['.key'])"><i class="far fa-trash-alt ph2 pointer"></i></span>
    ...
    save: function () {
      ...
      switch (this.mode) {
        case this.CONSTANTS.MODE.EDIT:
          this.meals.some(meal => {
            if (meal.description.toUpperCase() === this.meal.toUpperCase()) {
              namesRef.child(meal['.key']).update({description: this.meal, calories: this.calories});
              return true;
            } else {
              return false;
            }
        ...
        default:
          ...
          if (duplicated) {
            ...
            namesRef.push({ description: this.meal, calories: this.calories });
          }
      }
    },
    checkForm: function () {
